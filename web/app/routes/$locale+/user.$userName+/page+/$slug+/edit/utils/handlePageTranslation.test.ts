...
import { fetchPageWithSourceTexts } from "~/routes/$locale+/user.$userName+/page+/$slug+/functions/queries.server";
...
// Mock dependencies
vi.mock(
	"~/routes/$locale+/user.$userName+/page+/$slug+/functions/mutations.server",
	() => ({
...
vi.mock(
	"~/routes/$locale+/user.$userName+/page+/$slug+/functions/queries.server",
	() => ({

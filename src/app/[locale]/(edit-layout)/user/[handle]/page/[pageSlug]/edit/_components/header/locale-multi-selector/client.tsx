...
export function LocaleMultiSelector({
  ...
  const selectClassNames = {
    control: () =>
      cn(
        'min-h-[72px] w-full cursor-pointer rounded-lg border border-border bg-transparent px-4 text-sm',
    ...
    <Popover onOpenChange={setOpen} open={open}>
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn(
            'flex h-8 items-center gap-1 rounded-full px-3 hover:bg-secondary/80',
            className
          )}
          size="sm"
          variant="ghost"
        >
          ...
          {count > 0 && (
            <span className="min-w-[16px] rounded-full bg-primary px-1 text-center font-medium text-primary-foreground text-xs">
              {count}
            </span>
          )}
      ...
      <PopoverContent align="end" className="w-[300px] space-y-2 p-2">
        <Select
          classNamePrefix="rs"

...
明示された変更以外は既存仕様を維持し、勝手な簡素化・機能削除・挙動変更を行わない。

## フロント移行方針（確定）

- フロント移行では `origin/main` の既存ファイルを正本とし、URL・UI・shadcn の配置を維持したまま、既存パス上で TanStack 適応する。
- 全既存ファイルの移動・複製・ディレクトリ再編を禁止し、`src/routes` には不可避な最小 route 定義だけを置き、UIコンポーネントは元の `src/app/[locale]/.../_components` から import する。
- 旧 `/$locale/$handle/$pageSlug` と `/$locale/auth/login` の URL を維持し、新しい scripture 専用 URL は導入しない。
- 差分化するのは、トップページを仏典一覧にすること、記事投稿を廃止すること、記事コメントを廃止することの明示変更だけとする。
- 配置変更と再デザインは禁止する。
...
- 厳密な型定義のため､基本的に引数は必須のものを使う｡どうしても必要なときは `| null` で対応する。
